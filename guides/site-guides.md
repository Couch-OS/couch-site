...
`usage/` contains the public “Using Couch” guides. Link new guides from its
...
The landing-page demo and guide images both use the pinned Couch checkout's
`preview/`, which compiles `ui/couch-gui/ui/app.slint` and its production
components to WebAssembly.
...
to this repository's `tools/tests/site-usage.cjs`. Fixtures may supply example content and stop the
...
SITE_DIRECTORY=$PWD/dist COUCH_SOURCE=../couch NODE_PATH=/path/to/node_modules node tools/tests/site-usage.cjs
SITE_DIRECTORY=$PWD/dist COUCH_SOURCE=../couch NODE_PATH=/path/to/node_modules node tools/tests/site-demo.cjs
```
...
in `usage/screenshots/` in the rendered destination, then checks every guide at desktop and mobile widths.
...
```sh
SITE_URL=https://couch-os.dev/ \
  NODE_PATH=/path/to/node_modules node tools/tests/site-usage.cjs --check-only
```
