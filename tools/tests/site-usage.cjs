...
const siteDirectory=path.resolve(process.env.SITE_DIRECTORY||root);
const couchSource=process.env.COUCH_SOURCE&&path.resolve(process.env.COUCH_SOURCE);
const output=path.join(siteDirectory,'usage/screenshots');
...
(async()=>{
 ...
 try {
  if(generate){
   ...
   assert.ok(couchSource,'COUCH_SOURCE is required when generating screenshots');
   ...
   await fs.writeFile(path.join(output,'manifest.json'),JSON.stringify({schema:1,source_commit:execFileSync('git',['rev-parse','HEAD'],{cwd:couchSource,encoding:'utf8'}).trim(),renderer:'Production ui/couch-gui/ui/app.slint via preview/ WASM',fixture_source:'preview/src/demo.rs',images},null,2)+'\n');
  }
